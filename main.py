...
def filter_year_autor(dataset, year, autor):
    ...
    for line in dataset:
        ...
        autor_value = obj1["Book-Author"]
        year_value = obj2["Year-Of-Publication"]

        if  year_value < "1990":
            if autor_value == Name_Autor :
                filtered += [obj1]
...
def references(book, num_citas=20):
    ramdom_lines = random.sample(book, num_citas)
    
    citas = []
    for lines in ramdom_lines:
